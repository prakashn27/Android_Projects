    ...
    private boolean testQuery() {
        try {
            for (int i = 0; i < TEST_CNT; i++) {
                ...
                //should return only one row  so islast and isFirst should be same
                Log.i(TAG, "isFirst " + resultCursor.isFirst() + "   isLast " + resultCursor.isLast());
                Log.i(TAG, "get count: " + resultCursor.getCount() + " column Count" + resultCursor.getColumnCount());
